...
## 유명인 얼굴 인식 API {#CelebrityAPI}
입력받은 이미지로부터 얼굴을 감지하고 감지한 얼굴이 어떤 유명인가 닮았는지 분석하여 그 결과를 반환하는 REST API입니다.
...
## 얼굴 감지 API {#FaceAPI}
입력받은 이미지로부터 얼굴을 감지하고 입력된 이미지에서 얼마나 많은 얼굴이 감지되었고 각 얼굴이 어디에 어떤 크기로 위치하며 어떤 모습을 하고 있는지 반환하는 REST API입니다.
